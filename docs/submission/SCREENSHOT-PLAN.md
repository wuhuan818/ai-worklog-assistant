...
|---|---|---|---|---|
| 1 | `01-current-task-and-error.png` | 开始任务后运行 `node hello.js` | 编辑器、终端错误与 AI Worklog 侧栏同屏 | 自动关联真实任务与开发错误；不含个人路径或日志 |
| 2 | `02-bug-and-fix-verified.png` | 创建 Bug、修复并再次运行 | 修复行、`Hello, World!`、侧栏当前 Bug | Bug 生命周期覆盖真实修复；隐藏无关终端历史 |
...
| 7 | `07-published-knowledge-search.png` | 发布真实候选后搜索 `ReferenceError` | 知识搜索结果及标题 | 审核后的内容进入本地可检索知识；不显示本机路径 |
