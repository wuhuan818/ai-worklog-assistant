...
`POST /projects` 接收 `name` 和可选 `workspace_path`，同用户同标准化名称/Workspace 幂等返回已有项目。`POST /tasks` 接收 `name`、`project_id`（或兼容旧的 `project`）及可选描述、需求编号、标签；已有活动任务返回 HTTP 409。`GET /tasks/active` 无活动任务返回 JSON `null`。结束任务返回 `status=completed`、后端生成的 `ended_at` 和 `duration_seconds`；重复结束返回 409。
