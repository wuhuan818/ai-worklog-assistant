...
1. **准备环境**
   - 操作：安装 VS Code，运行 `scripts/build-backend.ps1`，将 `artifacts/backend/ai-worklog-server.exe` 通过 `scripts/build-extension.ps1` 放入插件 `server/` 目录，按 F5 启动 Extension Development Host。
   - 预期结果：插件成功加载，Activity Bar 出现 AI Worklog。
   - 截图：VS Code Extension Development Host 的 Activity Bar。
   - 失败日志：VS Code `Help > Toggle Developer Tools` Console；PowerShell 构建窗口。
