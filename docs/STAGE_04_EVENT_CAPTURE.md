...
激活时先创建后端管理器并注册 healthy 同步监听，再调用自动启动协调器；事件采集器和 Webview 不参与自动启动条件。自动启动日志使用 `[backend-auto-start]` 前缀，失败使用 `[backend-auto-start-error]`。此前 F5 后后端停留在“已停止”的真实根因是激活流程从未调用 `ServerManager.start()`，启动逻辑仅挂在手动按钮；现已修复。人工验收结果仍待用户确认。
