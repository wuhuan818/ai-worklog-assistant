...
ROOT=Path(__file__).resolve().parents[1]; EXE=ROOT/'artifacts/build/stage11b-temp/dist/ai-worklog-server-stage11b.exe'
def port():
...
def main():
    if not EXE.exists():raise RuntimeError('run disposable PyInstaller build first')
    with tempfile.TemporaryDirectory(prefix='stage11b-verify-') as raw:
        ...
        server=subprocess.Popen([str(EXE)],env={**env,'WORKLOG_PORT':str(backend_port),'WORKLOG_SESSION_TOKEN':token},stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
        try:
