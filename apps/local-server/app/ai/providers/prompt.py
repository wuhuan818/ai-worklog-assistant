...
from .structured import SUMMARY_SCHEMA_VERSION, normalize_evidence_refs, summary_json_schema
...
def build_summary_messages(context_package: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    refs = normalize_evidence_refs(context_package.get("provenance", {}).get("included_source_refs", []))
    allowed_refs = ", ".join(refs[:200]) or "(none; use empty evidence_refs)"
    user = (
...
def build_repair_messages(context_package: Dict[str, Any], validation_stage: str, field_paths: List[str], allowed_refs: List[Any], sanitized_output: str) -> List[Dict[str, str]]:
    ...
    refs = ", ".join(normalize_evidence_refs(allowed_refs)[:200]) or "(none; use empty evidence_refs)"
