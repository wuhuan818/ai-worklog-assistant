...
(async () => { let succeeded = false; try {
  ...
  const code = await new Promise((resolve, reject) => { const child = spawn(executable, args, { env: { ...process.env, STAGE071_E2E_REPORT: report }, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] }); const stdout = fs.createWriteStream(path.join(temp, 'vscode.stdout.log')); const stderr = fs.createWriteStream(path.join(temp, 'vscode.stderr.log')); child.stdout.pipe(stdout); child.stderr.pipe(stderr); child.on('error', reject); child.on('close', value => resolve(value ?? 1)); });
