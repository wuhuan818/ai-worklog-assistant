...
export interface GenerationCommandDeps { api(): Promise<ApiClient>; profiles: AiProfileStore; onStatus?(job: GenerationJob | undefined): void; }
...
export function registerGenerationCommands(context: vscode.ExtensionContext, deps: GenerationCommandDeps): void {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('aiWorklog.generateAiSummaryDraft', async () => {
    ...
    const api = await deps.api(); const task = await taskPicker(api); if (!task) return;
    ...
    try { await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Generating AI summary draft', cancellable: false }, async report => { progress = report; const created = await client.create(packageToSend.id, { profile_id: profile.id, provider: profile.provider, base_url: profile.baseUrl, model: profile.model, thinking_enabled: profile.thinkingEnabled, timeout_seconds: profile.timeoutSeconds, max_output_tokens: profile.maxOutputTokens, api_key: apiKey }, crypto.randomUUID()); runningJobId = created.job_id || created.id; if (!runningJobId) throw new Error('Backend did not return a generation job id'); deps.onStatus?.(created); const result = await client.wait(runningJobId, job => { deps.onStatus?.(job); progress?.report({ message: generationStatusLabel(job.status) }); }); if (result.status !== 'succeeded') throw new Error(result.error_summary || `Generation ${result.status}`); vscode.window.showInformationMessage('AI summary draft generated.'); await openDraft(task); }); } catch (error) { vscode.window.showErrorMessage(error instanceof Error ? error.message : String(error)); } finally { runningJobId = undefined; }
  }));
