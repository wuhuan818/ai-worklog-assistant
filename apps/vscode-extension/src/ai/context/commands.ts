...
export function registerContextCommands(context: vscode.ExtensionContext, deps: ContextCommandDeps): void {
  ...
  const open = async (rebuild: boolean) => {
    ...
    if (!preview || preview.isDisposed) {
      const panel = vscode.window.createWebviewPanel('aiWorklog.contextPreview', 'AI Context Preview', vscode.ViewColumn.Active, { enableScripts: true });
      const created = new PreviewPanel(panel, async contextId => {
        const ready = await (new ContextClient(await deps.api())).ready(contextId);
        if (preview === created && !created.isDisposed) created.show(ready);
        vscode.window.showInformationMessage('AI 上下文已标记为 Ready');
      }, () => { if (preview === created) preview = undefined; });
      preview = created; context.subscriptions.push(created);
    }
    if (!preview.isDisposed) preview.show(packageToShow);
  };
