...
class Provider implements vscode.WebviewViewProvider {
  ...
  publishGenerationState(job: GenerationJob | undefined): void { const fingerprint = generationFingerprint(job); if (fingerprint === this.generationFingerprint) return; this.generationFingerprint = fingerprint; this.generationStatusLabel = job ? `AI Summary: ${generationStatusLabel(job.status)}` : 'AI Summary: not generated'; if (this.currentView) void this.currentView.webview.postMessage({ type: 'generationState', label: this.generationStatusLabel }); }
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.html += `<script>const context=document.createElement('section');const contextSummary=document.createElement('p');contextSummary.textContent='AI Context：仅可预览已结束任务';const contextPreview=document.createElement('button');contextPreview.textContent='预览上下文';contextPreview.addEventListener('click',()=>vscode.postMessage({type:'previewAiContext'}));const generation=document.createElement('p');generation.id='generationState';generation.textContent='AI Summary: not generated';context.append(document.createElement('hr'),contextSummary,contextPreview,generation);document.body.appendChild(context);window.addEventListener('message',e=>{if(e.data.type==='generationState')generation.textContent=e.data.label;});</script>`;
    void view.webview.postMessage({ type: 'generationState', label: this.generationStatusLabel });
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  try {
    ...
    registerGenerationCommands(context, { api: () => ensureServer(server), profiles: aiProfiles, onStatus: job => providerView?.publishGenerationState(job) });
