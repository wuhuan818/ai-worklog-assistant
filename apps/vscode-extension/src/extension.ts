...
class Provider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.html += `<script>const ai=document.createElement('section');ai.innerHTML='<hr><p id="aiProvider">AI Provider：未配置</p><p id="aiProfile">AI Profile：-</p><p id="aiModel">模型：-</p><p id="aiThinking">思考模式：关闭</p><p id="aiKey">API Key：未配置</p><p id="aiConnection">连接状态：未测试</p><p id="aiEndpoint">Endpoint：-</p>';document.body.appendChild(ai);window.addEventListener('message',e=>{const m=e.data;if(m.type==='aiState'){aiProvider.textContent='AI Provider：'+m.provider;aiProfile.textContent='AI Profile：'+m.profile;aiModel.textContent='模型：'+m.model;aiThinking.textContent='思考模式：'+m.thinking;aiKey.textContent='API Key：'+m.apiKey;aiConnection.textContent='连接状态：'+m.connection;aiEndpoint.textContent='Endpoint：'+m.endpoint;}});</script>`;
    ...
    void this.aiProfiles.key(this.aiProfiles.current()?.id || '').then(key => view.webview.postMessage({ type: 'aiState', ...aiViewModel(this.aiProfiles.current(), Boolean(key)) }));
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  try {
    ...
    if (context.extensionMode === vscode.ExtensionMode.Test) {
      context.subscriptions.push(vscode.commands.registerCommand('aiWorklog.test.createAiProfile', async (input: { provider: AiProviderKind; baseUrl: string; model: string; apiKey?: string; id?: string }) => { const timestamp = new Date().toISOString(); const id = input.id || `test-ai-${Date.now()}`; const existing = aiProfiles.profiles().find(profile => profile.id === id); const profile: AiProviderProfile = { id, displayName: `Test ${input.provider}`, provider: input.provider, baseUrl: input.baseUrl, model: input.model, thinkingEnabled: false, timeoutSeconds: 5, maxOutputTokens: 16, createdAt: existing?.createdAt || timestamp, updatedAt: timestamp }; await aiProfiles.save(profile, input.apiKey); await aiProfiles.select(profile.id); return { ...profile, apiKey: undefined }; }));
      context.subscriptions.push(vscode.commands.registerCommand('aiWorklog.test.testAiConnection', async () => { const profile = aiProfiles.current(); if (!profile) throw new Error('No AI profile'); const key = await aiProfiles.key(profile.id); if (!key) throw new Error('No AI key'); return (await ensureServer(server)).testAiConnection({ provider: profile.provider, base_url: profile.baseUrl, model: profile.model, api_key: key, thinking_enabled: profile.thinkingEnabled, timeout_seconds: profile.timeoutSeconds, max_output_tokens: profile.maxOutputTokens }); }));
