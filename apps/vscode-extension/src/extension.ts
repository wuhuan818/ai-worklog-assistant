...
class Provider implements vscode.WebviewViewProvider {
  ...
  constructor(private readonly context: vscode.ExtensionContext, private readonly state: TaskState, private readonly server: ServerManager, private readonly controller: TaskLifecycleController, private readonly events: EventCaptureController, private readonly log: (message: string) => void = () => undefined) {}
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    const refreshView = async (reason: string) => { const renderVersion = ++this.renderVersion; if (reason === 'visible') this.log('[worklog-view] visible=true'); publishBackendState(renderVersion); publishTaskState(renderVersion); await publishEvents(renderVersion); };
    void refreshView('resolve');
    this.viewTimer = setInterval(() => { if (view.visible) void refreshView('timer'); }, 1000);
    const taskSubscription = this.state.onDidChange(() => void refreshView('task'));
    ...
    this.viewSubscriptions.push(taskStateSubscription, taskSubscription, visibilitySubscription);
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  try {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('aiWorklog.sidebar', new Provider(context, state, server, controller, events, message => logger.appendLine(message))));
