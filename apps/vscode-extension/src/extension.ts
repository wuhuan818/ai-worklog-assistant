...
class Provider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.html += `<script>const context=document.createElement('section');const contextSummary=document.createElement('p');contextSummary.textContent='AI Context：仅可预览已结束任务';const contextPreview=document.createElement('button');contextPreview.textContent='预览上下文';contextPreview.addEventListener('click',()=>vscode.postMessage({type:'previewAiContext'}));context.append(document.createElement('hr'),contextSummary,contextPreview);document.body.appendChild(context);</script>`;
