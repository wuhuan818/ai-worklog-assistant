...
import { AiProfileStore, AiProviderKind, AiProviderProfile, EmbeddingProfileStore, EmbeddingProfile, defaultsFor, qwenRegionOptions, qwenWorkspaceBaseUrl } from './ai/profileStore';
...
class Provider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.html += `<script>const knowledge=document.createElement('section');const knowledgeSearch=document.createElement('button');knowledgeSearch.textContent='搜索知识';knowledgeSearch.onclick=()=>vscode.postMessage({type:'searchKnowledge'});const knowledgeAsk=document.createElement('button');knowledgeAsk.textContent='问知识库';knowledgeAsk.onclick=()=>vscode.postMessage({type:'askKnowledge'});knowledge.append(document.createElement('hr'),document.createTextNode('知识库'),document.createElement('br'),knowledgeSearch,knowledgeAsk);document.body.appendChild(knowledge);</script>`;
    ...
    this.viewSubscriptions.push(view.webview.onDidReceiveMessage(async message => {
      try {
        ...
        if (message.type === 'searchKnowledge') await vscode.commands.executeCommand('aiWorklog.searchKnowledge');
        if (message.type === 'askKnowledge') await vscode.commands.executeCommand('aiWorklog.askKnowledge');
...
async function searchKnowledge(server: ServerManager): Promise<void> {
  const input = await promptText({ prompt: '搜索已发布知识', placeHolder: '例如：登录、SQLite、Android' });
  if (input.kind === 'cancelled' || !input.value.trim()) return;
  const result = await (await ensureServer(server)).searchKnowledge(input.value.trim());
  if (!result.items.length) { vscode.window.showInformationMessage('未找到匹配的已发布知识。'); return; }
  const choice = await vscode.window.showQuickPick(result.items.map(item => ({ label: item.title, description: item.category, detail: item.snippet, item })), { title: `知识搜索：${input.value.trim()}`, matchOnDescription: true, matchOnDetail: true });
  if (!choice) return;
  const root = vscode.Uri.file(path.join(server.dataDir, 'knowledge')).fsPath;
  const candidate = path.resolve(root, ...choice.item.logical_path.split('/'));
  if (!candidate.startsWith(root + path.sep)) { vscode.window.showErrorMessage('知识文件路径校验失败。'); return; }
  try { await vscode.window.showTextDocument(await vscode.workspace.openTextDocument(vscode.Uri.file(candidate))); }
  catch { vscode.window.showErrorMessage('已发布知识文件无法打开。'); }
}
function embeddingPayload(profile: EmbeddingProfile, apiKey: string) { return { id: profile.id, name: profile.name, kind: profile.kind, base_url: profile.baseUrl, model: profile.model, dimensions: profile.dimensions, timeout_seconds: profile.timeoutSeconds, enabled: profile.enabled, api_key: apiKey }; }
async function configureEmbedding(store: EmbeddingProfileStore): Promise<void> {
  const choice = await promptPick([{label:'Qwen',embeddingKind:'qwen' as const},{label:'Custom OpenAI-compatible',embeddingKind:'openai-compatible' as const}],{title:'Embedding Provider'}); if(choice.kind==='cancelled')return;
  const name=await promptText({prompt:'Embedding Profile 名称',value:choice.value.label}); if(name.kind==='cancelled'||!name.value.trim())return;
  const base=await promptText({prompt:'Embedding Base URL',value:choice.value.embeddingKind==='qwen'?'https://dashscope.aliyuncs.com/compatible-mode/v1':''}); if(base.kind==='cancelled'||!base.value.trim())return;
  const model=await promptText({prompt:'Embedding 模型',value:choice.value.embeddingKind==='qwen'?'text-embedding-v4':''}); if(model.kind==='cancelled'||!model.value.trim())return;
  const key=await promptText({prompt:'Embedding API Key',password:true}); if(key.kind==='cancelled'||!key.value)return;
  const current=store.current(); await store.save({id:current?.id||randomUUID(),name:name.value.trim(),kind:choice.value.embeddingKind,baseUrl:base.value.trim().replace(/\/+$/,''),model:model.value.trim(),timeoutSeconds:30,enabled:true},key.value); vscode.window.showInformationMessage('Embedding Profile 已安全保存。启用语义索引会将已发布知识发送到该 Provider。');
}
async function testEmbedding(store: EmbeddingProfileStore,server:ServerManager): Promise<void>{const profile=store.current();if(!profile){vscode.window.showWarningMessage('请先配置 Embedding Provider。');return;}const key=await store.key(profile.id);if(!key){vscode.window.showWarningMessage('Embedding API Key 未配置。');return;}const result=await(await ensureServer(server)).testEmbedding(embeddingPayload(profile,key));vscode.window.showInformationMessage(`Embedding 连接成功：${result.model}，${result.dimensions} 维，${result.latency_ms}ms`);}
async function rebuildSemanticIndex(store:EmbeddingProfileStore,server:ServerManager):Promise<void>{const profile=store.current();const key=profile&&await store.key(profile.id);if(!profile||!key){vscode.window.showWarningMessage('请先配置 Embedding Provider。');return;}const confirm=await vscode.window.showWarningMessage('建立语义索引会将已发布知识发送到你配置的 Embedding Provider。API Key 不会写入数据库。',{modal:true},'建立索引');if(confirm!=='建立索引')return;await vscode.window.withProgress({location:vscode.ProgressLocation.Notification,title:'建立语义知识索引'},async()=>{const result=await(await ensureServer(server)).rebuildSemanticIndex(embeddingPayload(profile,key));vscode.window.showInformationMessage(`语义索引已完成：${result.indexed_chunks}/${result.total_chunks} chunks。`);});}
async function askKnowledge(store:EmbeddingProfileStore,chat:AiProfileStore,server:ServerManager):Promise<void>{const input=await promptText({prompt:'问知识库',placeHolder:'输入一个关于已发布知识的问题'});if(input.kind==='cancelled'||!input.value.trim())return;const chatProfile=chat.current();const chatKey=chatProfile&&await chat.key(chatProfile.id);if(!chatProfile||!chatKey){vscode.window.showWarningMessage('请先配置 AI Chat Provider。');return;}const embedding=store.current();const embeddingKey=embedding&&await store.key(embedding.id);const mode: 'lexical'|'hybrid'=embedding&&embeddingKey?'hybrid':'lexical';const answer=await vscode.window.withProgress({location:vscode.ProgressLocation.Notification,title:'正在检索知识并生成回答'},()=> (async()=>{return (await ensureServer(server)).ragAnswer({query:input.value.trim(),mode,limit:6,embedding_profile:embedding&&embeddingKey?embeddingPayload(embedding,embeddingKey):undefined,chat_profile:{profile_id:chatProfile.id,provider:chatProfile.provider,base_url:chatProfile.baseUrl,model:chatProfile.model,thinking_enabled:chatProfile.thinkingEnabled,timeout_seconds:chatProfile.timeoutSeconds,max_output_tokens:chatProfile.maxOutputTokens,api_key:chatKey}});})());const citations=answer.citations.map((citation,index)=>{const item=answer.source_summaries.find(source=>source.source_ref===citation.source_ref);return item?`${index+1}. ${item.title}`:'';}).filter(Boolean).join('\n');const message=`${answer.answer}\n\n引用：\n${citations||'（证据不足）'}\n\n检索方式：${answer.effective_retrieval_mode}`;const action=await vscode.window.showInformationMessage(message,'打开引用');if(action==='打开引用'&&answer.citations[0]){const item=answer.source_summaries.find(source=>source.source_ref===answer.citations[0].source_ref);if(item){const root=vscode.Uri.file(path.join(server.dataDir,'knowledge')).fsPath;const target=path.resolve(root,...item.logical_path.split('/'));if(target.startsWith(root+path.sep))await vscode.window.showTextDocument(await vscode.workspace.openTextDocument(vscode.Uri.file(target)));}}}
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  try {
    ...
    const embeddingProfiles = new EmbeddingProfileStore(context.globalState, context.secrets);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('aiWorklog.searchKnowledge', () => run(() => searchKnowledge(server))));
    context.subscriptions.push(vscode.commands.registerCommand('aiWorklog.configureEmbedding', () => run(() => configureEmbedding(embeddingProfiles))));
    context.subscriptions.push(vscode.commands.registerCommand('aiWorklog.testEmbedding', () => run(() => testEmbedding(embeddingProfiles,server))));
    context.subscriptions.push(vscode.commands.registerCommand('aiWorklog.rebuildSemanticIndex', () => run(() => rebuildSemanticIndex(embeddingProfiles,server))));
    context.subscriptions.push(vscode.commands.registerCommand('aiWorklog.askKnowledge', () => run(() => askKnowledge(embeddingProfiles,aiProfiles,server))));
