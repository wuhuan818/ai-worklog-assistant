...
export function activate(context: vscode.ExtensionContext): void {
  ...
  try {
    ...
    if (context.extensionMode === vscode.ExtensionMode.Test) {
      context.subscriptions.push(vscode.commands.registerCommand('aiWorklog.test.createAiProfile', async (input: { provider: AiProviderKind; baseUrl: string; model: string; apiKey: string }) => { const timestamp = new Date().toISOString(); const profile: AiProviderProfile = { id: `test-ai-${Date.now()}`, displayName: `Test ${input.provider}`, provider: input.provider, baseUrl: input.baseUrl, model: input.model, thinkingEnabled: false, timeoutSeconds: 5, maxOutputTokens: 16, createdAt: timestamp, updatedAt: timestamp }; await aiProfiles.save(profile, input.apiKey); await aiProfiles.select(profile.id); return { ...profile, apiKey: undefined }; }));
